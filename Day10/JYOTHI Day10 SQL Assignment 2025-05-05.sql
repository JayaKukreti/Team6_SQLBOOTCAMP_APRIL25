...
drop function get_total_stock_value;
CREATE OR REPLACE FUNCTION get_total_stock_value(v_category_name TEXT)
...
BEGIN
    RETURN ROUND(
        (
            ...
            WHERE c.category_name = v_category_name
        ), 2
    );
END;
$$ 
...
SELECT get_total_stock_value('Drinks');

SELECT get_total_stock_value('Condiments');

SELECT get_total_stock_value('Dairy Products');

SELECT get_total_stock_value('Seafood');

SELECT get_total_stock_value('Meat/Poultry');
